...
export class ProfileService {
    ...
    constructor(io: IOService) {
        ...
        this.io.readDirSync('./profiles/').forEach(profile => {
            if (this.io.lstatSync(`./profiles/${profile}`).isDirectory()) {
                try {
                    p.push(new Profile(this.io.deserialize(this.io.readFileSync(`./profiles/${profile}/profile.json`)) as Record<string, any>))
                } catch (_) {
    ...
    getProfileById(id: string): Profile {
        return this.profiles.filter(profile => { profile.character._id === id })[0]
    }
